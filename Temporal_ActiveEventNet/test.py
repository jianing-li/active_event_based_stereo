"""
Function: ActiveEventNet+ introduces an efficient temporal consistency architecture that use rich spatiotemporal cues from event streams.
...
parser.add_argument('--temporal_sequence_length', type=int, default=3, help='temporal sequence length for sequence modeling')
...
test_dataset = StereoDataset(args.datapath, args.testlist, False, args.temporal_sequence_length)
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        for idx, disp_est in enumerate(disp_ests[-1]):
            ...
            if float(args.save_frame) == 1:
                ...
                # disparity_map = cv2.applyColorMap(cv2.convertScaleAbs(disp_est, alpha=15), cv2.COLORMAP_JET)  # DAVIS
...
def test_sample(sample):
    ...
    if imgL.dim() == 4:
        imgL = imgL.permute(1, 0, 2, 3).unsqueeze(0)
        imgR = imgR.permute(1, 0, 2, 3).unsqueeze(0)
    imgL = imgL.cuda()
    imgR = imgR.cuda()
