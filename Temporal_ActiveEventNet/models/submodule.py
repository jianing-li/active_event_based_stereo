...
class Gate_Dynamic_Interweave_Module(nn.Module):
    ...
    def forward(self, ref, tgt):
        ...
        if ref.dim() == 4:  # Single bin: [B, C, H, W]
            return self._forward_single(ref, tgt)
        elif ref.dim() == 5:  # Temporal bins: [B, T, C, H, W]
            return self._forward_temporal(ref, tgt)
        else:
            raise ValueError(f"Unsupported input dimension: {ref.dim()}. Expected 4 or 5 dimensions.")

    def _forward_single(self, ref, tgt):
        """Handle single bin input: [B, C, H, W]"""
        B, C, H, W = ref.shape
        concat = torch.cat([ref, tgt], dim=1)
        gates = self.gate_gen(concat)

        gated_ref = ref * gates
        gated_tgt = tgt * (1 - gates)

        interwoven = torch.zeros(B, 2 * C, H, W, device=ref.device)
        interwoven[:, ::2, :, :] = gated_ref
        interwoven[:, 1::2, :, :] = gated_tgt
        return interwoven.contiguous()

    def _forward_temporal(self, ref, tgt):
        """Process temporal bin inputs: [B, T, C, H, W] """
        B, T, C, H, W = ref.shape

        # Reshape to process all bins at once: [B*T, C, H, W]
        ref_reshaped = ref.view(B * T, C, H, W)
        tgt_reshaped = tgt.view(B * T, C, H, W)

        # Process using 2D gate generator
        concat = torch.cat([ref_reshaped, tgt_reshaped], dim=1)
        gates = self.gate_gen(concat)

        gated_ref = ref_reshaped * gates
        gated_tgt = tgt_reshaped * (1 - gates)

        # Create interwoven output
        interwoven = torch.zeros(B * T, 2 * C, H, W, device=ref.device)
        interwoven[:, ::2, :, :] = gated_ref
        interwoven[:, 1::2, :, :] = gated_tgt

        # Reshape back to temporal format: [B, T, 2*C, H, W]
        interwoven = interwoven.view(B, T, 2 * C, H, W)
        return interwoven.contiguous()
